...
public class Extract{
	...
	public static void GetEmployeeActivities(){
		...
		ParseDevice("device.csv");	
		ParseHttp("http.csv");
		ParseLogon("logon.csv");
		ParseEmail("email.csv");
		ParseFile("file.csv");
	...
	public static void ParseDevice(String fileName){
        ...
        try {
			SimpleDateFormat formatter = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");  //1/2/2010 7:21:00 AM
            ...
            while ((line = fileReader.readLine()) != null) {
                ...
                if (tokens.length > 0) {
					...
					//Date date = new Date(Long.parseLong(tokens[1])*1000);
					/*
					if(date.getYear() == 111){
						break;
					}
